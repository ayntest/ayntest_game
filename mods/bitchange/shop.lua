...
--  https://forum.core.net/viewtopic.php?id=7002
--License: WTFPL
...
local function get_exchange_shop_formspec(number,pos,title)
	...
	elseif(number == 2 or number == 3) then
		...
		formspec = ("size[11,10;]"..
				...
				"button[4.2,0.25;1,0.5;set_title;Set]"..
...
core.register_on_player_receive_fields(function(sender, formname, fields)
	...
	if(fields.exchange) then
		...
		if not shop_inv:is_empty("cust_ej") or not shop_inv:is_empty("custm_ej") then
			core.chat_send_player(player_name, "Exchange shop: Error, ejected items detected!")
		end
		...
		if(err_msg ~= "") then
			core.chat_send_player(player_name, "Exchange shop: "..err_msg)
		end
	elseif(fields.vstock and bitchange_has_access(shop_owner, player_name) and not fields.quit) then
		core.show_formspec(sender:get_player_name(),"bitchange:shop_formspec",get_exchange_shop_formspec(3, pos, title))
	elseif(fields.vcustm and bitchange_has_access(shop_owner, player_name) and not fields.quit) then
		core.show_formspec(sender:get_player_name(),"bitchange:shop_formspec",get_exchange_shop_formspec(2, pos, title))
	end
end)

core.register_node("bitchange:shop", {
	description = "Shop",
	...
	can_dig = function(pos,player)
		...
		if(inv:is_empty("stock") and inv:is_empty("custm") and 
			...
			core.chat_send_player(player:get_player_name(), "Can not dig exchange shop, one or multiple stocks are in use.")
			return false
		end
	end,
    ...
    allow_metadata_inventory_put = function(pos, listname, index, stack, player)
		...
		if listname == "custm" then
			core.chat_send_player(player:get_player_name(), "Exchange shop: Please press 'Customers stock' and insert your items there.")
			return 0
		end
		local meta = core.get_meta(pos)
