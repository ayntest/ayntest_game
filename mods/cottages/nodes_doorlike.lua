...
cottages_window_sutter_operate = function( pos, old_node_state_name, new_node_state_name )
	...
	for i,v in ipairs(offsets) do
		...
		if( node and node.name and node.name==old_node_state_name 
			...
			core.add_node({x=pos.x, y=(pos.y+v), z=pos.z },
...
core.register_node("cottages:window_shutter_open", {
	...
	on_rightclick = function(pos, node, puncher)
		core.add_node(pos, {name = "cottages:window_shutter_closed", param2 = node.param2})
		cottages_window_sutter_operate( pos, "cottages:window_shutter_open", "cottages:window_shutter_closed" );
	end,
	is_ground_content = false,
})

core.register_node("cottages:window_shutter_closed", {
	...
	on_rightclick = function(pos, node, puncher)
		core.add_node(pos, {name = "cottages:window_shutter_open", param2 = node.param2})
...
minetest.register_abm({
	...
	action = function(pos)
		...
		if( not(core.get_timeofday() < 0.2 or core.get_timeofday() > 0.805)) then
		   ...
		   core.add_node(pos, {name = "cottages:window_shutter_open", param2 = old_node.param2})
		   cottages_window_sutter_operate( pos, "cottages:window_shutter_closed", "cottages:window_shutter_open" );
	   end
	end
})
...
minetest.register_abm({
	...
	action = function(pos)
		...
		if( core.get_timeofday() < 0.2 or core.get_timeofday() > 0.805) then
		   local old_node = core.get_node( pos );
		   core.add_node(pos, {name = "cottages:window_shutter_closed", param2 = old_node.param2})
		   cottages_window_sutter_operate( pos, "cottages:window_shutter_open", "cottages:window_shutter_closed" );
		end
	end
})
...
------------------------------------------------------------------------------------------------------------------------------
core.register_node("cottages:half_door", {
	...
	on_rightclick = function(pos, node, puncher)
		...
		core.add_node(pos, {name = "cottages:half_door", param2 = param2})
		...
		if( node2 ~= nil and node2.name == node.name and node2.param2==node.param2) then
		   core.add_node( {x=pos.x,y=(pos.y+1),z=pos.z}, {name = "cottages:half_door", param2 = param2})
		end
	end,
	is_ground_content = false,
})

core.register_node("cottages:half_door_inverted", {
	...
	on_rightclick = function(pos, node, puncher)
		...
		core.add_node(pos, {name = "cottages:half_door_inverted", param2 = param2})
		-- open upper parts of this door (if there are any)
		if( node2 ~= nil and node2.name == node.name and node2.param2==node.param2) then
		   core.add_node( {x=pos.x,y=(pos.y+1),z=pos.z}, {name = "cottages:half_door_inverted", param2 = param2})
		end
	end,
	is_ground_content = false,
})
...
------------------------------------------------------------------------------------------------------------------------------
core.register_node("cottages:gate_closed", {
	...
	on_rightclick = function(pos, node, puncher)
		core.add_node(pos, {name = "cottages:gate_open", param2 = node.param2})
	end,
	is_ground_content = false,
})

core.register_node("cottages:gate_open", {
	...
	on_rightclick = function(pos, node, puncher)
		core.add_node(pos, {name = "cottages:gate_closed", param2 = node.param2})
	end,
	is_ground_content = false,
})
...
cottages.register_hatch = function( nodename, description, texture, receipe_item )
	core.register_node( nodename, {
		...
		on_rightclick = function(pos, node, puncher)
			core.add_node(pos, {name = node.name, param2 = new_facedirs[ node.param2+1 ]})
		end,
		is_ground_content = false,
	})
