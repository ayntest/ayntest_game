...
-- just straw
core.register_node("cottages:straw", {
	drawtype = "normal",
	description = 'Straw',
...
core.register_node("cottages:threshing_floor", {
	drawtype = "nodebox",
...
-- TODO: stone also looks pretty well for this
	...
	after_place_node = function(pos, placer)
		...
		meta:set_string("infotext", ("Threshing floor (owned by %s)"):format(meta:get_string("owner") or ""));
	...
	on_punch = function(pos, node, puncher)
		...
		if( ( stack1:is_empty()  and stack2:is_empty())
			...
			core.chat_send_player( name, 'One of the input slots contains something else than wheat or there is no wheat at all.' )
			return
		end

		-- on average, process 25 wheat at each punch (10..40 are possible)
		local anz_wheat = 10 + math.random( 0, 15 )
		...
		if( inv:room_for_item('straw','cottages:straw_mat '..tostring( anz_straw ))
			...
			if( anz_left > 0 ) then
				core.chat_send_player( name, ('You have threshed %s wheat (%s are left).'):format(anz_wheat,anz_left))
			else
				core.chat_send_player( name, ('You have threshed the last %s wheat.'):format(anz_wheat))
			end
		end	
	end,
})
...
core.register_node("cottages:handmill", {
	drawtype = "nodebox",
	...
	after_place_node = function(pos, placer)
		local meta = core.get_meta(pos);
		meta:set_string("owner", placer:get_player_name() or "");
		meta:set_string("infotext", ('Mill (powered by punching) (owned by %s)'):format(meta:get_string("owner") or ""));
                meta:set_string("formspec",
                               "size[8,8]"..
	...
	on_punch = function(pos, node, puncher)
		...
		if(    inv:room_for_item('flour','farming:flour '..tostring( anz ))) then
			...
			if( anz_left > 0 ) then
				core.chat_send_player( name, ('You have grinded %s wheat seeds (%s are left).'):format(anz,anz_left));
			else
				core.chat_send_player( name, ('You have grinded the last %s wheat seeds.'):format(anz));
			end

			-- if the version of MT is recent enough, rotate the mill a bit
			if( core.swap_node ) then
