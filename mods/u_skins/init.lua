...
minetest.register_chatcommand('setskin', {
	params = '<skin_id> [name]',
	description = 'Sets skin for user',
	privs = {
		server = true
	},
	func = function(name, param)
		local skin_id, pname = param:match("([0-9]+) *(%w*)")
		if ( pname ~= nil and minetest.get_player_by_name( pname ) ~= nil ) then
			name = pname
		elseif ( pname ~= '' ) then
		...
		minetest.chat_send_player(name, name ..  '\'s skin changed. Before: ' .. pskin .. ', now: '..skin_id)
	end
})
