	...
	on_place = function(itemstack, placer, pointed_thing)
	   ...
	   local keys=placer:get_player_control()
		...
		if( pointed_thing.type ~= "node" ) then
			core.chat_send_player( name, "  Error: No node selected.")
			return nil
		end

		local pos  = core.get_pointed_thing_position( pointed_thing, under )
		local node = core.get_node_or_nil( pos )

		--core.chat_send_player( name, "  Target node: "..core.serialize( node ).." at pos "..core.serialize( pos ).."."); 
		...
		core.chat_send_player( name, "Node replacement tool set to: '"..item[ "metadata" ]); 
...
replacer.replace = function( itemstack, user, pointed_thing, mode )
	...
	--core.chat_send_player( name, "You USED this on "..core.serialize( pointed_thing )..".")

	if( pointed_thing.type ~= "node" ) then
	  core.chat_send_player( name, 'You are pointing at the air')
	  return nil
	end
	...
	if( node == nil ) then
		core.chat_send_player( name, "Error: Target node not yet loaded. Please wait a moment for the server to catch up.")
		return nil
	end
	...
	--core.place_node( pos, { name =  item[ "metadata" ] } )
	core.add_node( pos, { name =  daten[1], param1 = daten[2], param2 = daten[3] } )
