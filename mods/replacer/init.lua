...
replacer.replace = function( itemstack, user, pointed_thing, mode )
	...
	if( pointed_thing.type ~= "node" ) then
	  minetest.chat_send_player( name, 'You are pointing at the air');
	  return nil;
	end
	...
	if( not(minetest.setting_getbool("creative_mode") )) then
	  ...
	  if( not( user:get_inventory():contains_item("main", daten[1]))) then
		 ...
		 minetest.chat_send_player( name, 'You have no further '..( daten[1] or "?" ));
		 return nil;
	  end
	...
	if node.name == 'default:obsidian' then
		minetest.chat_send_player( name, 'Unable to replace '..( node.name or "?" ));
		return nil
