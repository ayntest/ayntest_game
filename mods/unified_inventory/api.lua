...
function unified_inventory.go_spawn(player)
	local pos = minetest.setting_get_pos('static_spawnpoint')
	if minetest.setting_get('variable_spawnpoint') then
		local lim = tonumber(minetest.setting_get('variable_spawnpoint'))
		if lim > 0 then
			pos.x = pos.x + math.random(-(lim), lim)
			pos.z = pos.z + math.random(-(lim), lim)
		end
	end
	player:setpos( pos )
end
